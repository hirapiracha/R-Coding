...
#View(jan_feb) 
unique(not_jan$month) # $ selects the column 
...
-   Check what happens to the following 4 values, where the first value
    is the starting and the second the landing time:
...
How to convince ourselves that the two are the same? Use `all_equal`.

```{r compare-mystery-filters}
all_equal(mystery_filter, mystery_filter2)
```
...
minutes delay, but arrived with more than 120 minutes delay.
...
flights |> 
  arrange(air_time) |> 
  select(air_time, origin, destination) # Fix in class to `dest`
```
...
If you want columns that satisfy both conditions (not one or the other), then
you need two calls or use the logical `&`:
